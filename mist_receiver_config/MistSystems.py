...
def LogInCredentials(session, email, password):
    ...
    req_url = "https://" + base_url + LogInCredentials_url #https://api.mist.com/api/v1/login
...
def CreateWebhook_splunk(site_id, webhook_url, auth_token, splunk_token):
    api_url = "/api/v1/sites/" + site_id + "/webhooks"
    request_url = "https://" + base_url + api_url
    print(request_url)
    header = {"content-type": "application/json", 'Authorization': 'token {}'.format(auth_token)}
...
def mist_Delete(mist_url_extention, mist_auth_token):
    mist_session = requests.Session()
    mist_req_url = "https://" + base_url + mist_url_extention
    header = {"content-type": "application/json", 'Authorization': 'token {}'.format(mist_auth_token)}
    response = mist_session.delete(mist_req_url, headers=header, data={}, verify=False)
    return response

def DeleteWebhook(site_id, webhook_id, auth_token):
    api_url = "/api/v1/sites/" + site_id + "/webhooks/" + webhook_id
    request_url = "https://" + base_url + api_url
    print(request_url)
    header = {"content-type": "application/json", 'Authorization': 'token {}'.format(auth_token)}
    response = mist_Delete(api_url, auth_token)
    if response.status_code == 200:
        pass
    else:
        print("Error: " + str(response.status_code))
